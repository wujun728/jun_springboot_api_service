...
$(function(){
	$('#infoList').datagrid({
		...
		url:"<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=match_mvc_rule_list&groovySubName=getInfoList4Page",
		columns:[[
					{
						field : 'id',
						title : 'id',
	...
	function doPublish(){  
		...
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=match_mvc_rule_list&groovySubName=execPublishDefaultRule";
	...
	function addOne(){
		...
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=match_mvc_rule_list&groovySubName=createInfo";
	...
	function updateOne(){
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=match_mvc_rule_list&groovySubName=updateInfo";
	...
	function remove(){
		...
		if(sels == ''|| sels==null){
			...
			if(result == true){
				var querydata = $('#searchForm').serializeObject();
				var id = sels.id;
				var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=match_mvc_rule_list&groovySubName=delInfo";
				url=url+"&id="+id;
				$.post(url,function(data,stats){
							if(stats=="success" ){
								$.messager.show({
		...
		<form id="updateOneForm" novalidate method="post" action="">
			...
			<table id="updateTable"
				...
				<tr>
					...
					<td><input type="text" id="rule_id" name="rule_id" value="" /></td>
				</tr>
				<tr>
				...
				<tr>
					<td align="right">算法名称：</td>
					<td><input type="text" id="groovy_name" name="groovy_name" value="" /></td>
				</tr>				
				<tr>
					<td align="right">规则参数：</td>
					<td><textarea rows="10" cols="50"  id="rule_param" name="rule_param" ></textarea></td>
				</tr>
				<tr>
	...
	<div id="addOne" class="easyui-dialog" modal="true" align="center"
		...
		<form id="addForm" novalidate method="post" action="">
			...
			<table id="addTable" style="margin-top: 10px; margin-left: -40px;">
				...
				<tr>
					<td align="right">规则参数：</td>
					<td><input type="text" id="rule_param" name="rule_param" value="" /></td>
				</tr>
				<tr>
					<td align="right">备注：</td>
					<td><input type="text" id="remark" name="remark" value="" /></td>
